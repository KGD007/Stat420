...
**I used Plot and Breusch-Pagan to check the constant variance of the model. The Result is 0.2594. Hence we fail to reject null hypothesis that the error as constant variance about the model at alpha 0.05.I do not feel that the constant variance assumption has been violated for this model.**
...
**Used the Shapiro-Wilk normality to test for normality of the errors about the model. The result is 0.7721. Hence we fail to reject the null hypothesis that the errors are from a normal distribution. Same we can check from Histogram of residual where residuals are not in Normal form and in Q-Q plots as well line is not overlapping residuals. Hence I do not feel as if the normality assumption has been violated in this model after performing this test**
...
**When we Compare original model with high influence observations removed, we see slight differences in the coefficients,but ‘lcp’ and ‘gleason has significant differences,which can be the result of containing influential values. I also see a huge change in the negative value of Intercept.**
...
**For Model_A we see that the proportions of values are very close to the α (alpha) we are checking which holds true for the model which is simulated without violating any assumptions. However, Model_B we do not	see p-value proportions close to the α values.This shows that when simulating data that violates our model assumptions, our assumptions will not hold.**
...
**After seeing the plots we interpret that model with degree two visibly does not have constant fitted vs residual values, third and fourth seems to be better but in order to choose the best one we perform some tests - BP test results ploy3 = 0.1848, has equal variance as all fail to reject the null hypothesis that variances are equal. Shapiro-Wilk test, we get a resulting p-value of 0.9085 which means we fail to reject and can say that the errors from the poly 3 model are normally distributed - At last Cooks Distance test, there were no influential points for model poly 3. Hence we conclude that Poly 3 is the best acceptable model.**
***
...
**This is the best model for normality assumption and homoskedasticity compared to first and the second one,fitted vs residuals plot looks much more linear and possibly equal variance.The Q-Q plot for normality looks much better, however there is still some outward tails.**
...
**A diamond of 3 carats with a 99% prediction interval will cost $29429 with upper and lower boundary of $57894 and $14959**