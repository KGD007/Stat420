...
**We can see slope of Truck is more positive than car and both which shows by increasing horsepower in truck Co2 change is more than car and both type. In the same way Both has more change in CO2 than Car by increasing Horsepower. Same can be by scatter plots. hence this model fits better than Additive model***
...
**Since the p-value from ANOVA test is smaller than given α value, So we reject Null Hypothesis and prefer interaction model.**
...
**R chooses LOW as a reference level for Education, LOW as reference level for Income and Male for Sex variables**
...
```
**Since p-value is greater than Alpha value so we fail to reject Null Hypothesis. Hence I will prefer Additive Model**
...
```
**Since p-value is less than Alpha value so we reject Null Hypothesis. Hence I will prefer interactive Model**
...
```
```{r}
days_init = lm(Days ~ Pressure * Blood * Rate, data = hospital)
```

```{r}
anova(days_add, days_init)
```
...
- The value of the test statistic : **2.043** 
- The p-value of the test : **0.08705**
- A statistical decision using a significance level of $\alpha = 0.10$ : **Successfully able to reject Null Hypothesis**
- Which model you prefer : **We would prefer Big Model**
...
#### Y = β0+β1x1+β2∗139+β4∗10∗139+(β3+β5∗10+β6∗139+β7∗10∗139)x3+e
...
all.equal(lm_p, tt_p)
```
**The result of this code is TRUE which means that both calculations/values are equal. This means that checking for H0:β1=0 in a model with one dummy variable as was provided is identical to checking for a significant differerence in the mean between the two groups had they been separated (i.e. not in a linear regression model.**
